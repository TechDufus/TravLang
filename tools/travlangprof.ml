...
let idprefix = "__travlang_prof_"
let modprefix = "travlang__prof_"
...
let modes = ref "fm"
let dumpfile = ref "travlangprof.dump"
...
let usage = "Usage: travlangprof <options> <files>\noptions are:"

let print_version () =
  printf "travlangprof, version %s@." Sys.travlang_version;
  exit 0
  ...
  try
    ...
    Arg.parse_expand [
       "-f", Arg.String (fun s -> dumpfile := s),
             "<file>     Use <file> as dump file (default travlangprof.dump)";
