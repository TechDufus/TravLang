...
if test -e '.git' ; then :
  if test -z "$ac_read_git_config" ; then :
    extra_args=$(git config travlang.configure 2>/dev/null)
    extended_cache=$(git config travlang.configure-cache 2>/dev/null)
    cache_file=

    # If travlang.configure-cache is set, parse the command-line for the --host
    # option, in order to determine the name of the cache file.
    if test -n "$extended_cache" ; then :
      echo "Detected Git configuration option travlang.configure-cache set to \
\"$extended_cache\""
      dashdash=
      prev=
      host=default
      ...
      cache_file="`dirname "$0"`/$extended_cache/travlang-$host.cache"
    fi

    # If either option has a value, re-invoke configure
    if test -n "$extra_args$cache_file" ; then :
      echo "Detected Git configuration option travlang.configure set to \
\"$extra_args\""
      ...
      # but allows a single expansion of travlang.configure
