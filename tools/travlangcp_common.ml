...
module type travlangcp_args = sig
  val _a : unit -> unit
...
module type travlangCP = sig
  val bytecode : bool
  module Make_options : travlangcp_args -> Main_args.Arg_list
end

module Make(T: travlangCP) = struct
  let name = if T.bytecode then "travlangcp" else "travlangoptp"
  ...
  let main () =
    ...
    let status =
      ...
      Printf.ksprintf Sys.command
        "%s -pp \"travlangprof -instrument %s\" -I +profiling %s %s"
          (if T.bytecode then "travlangc" else "travlangopt")
