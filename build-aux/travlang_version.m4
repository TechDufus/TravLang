...
# travlang. It first defines the basic components and then computes all
# the different variants of the version used across the build system.

# For the M4 macros defined below, we use the travlang__ (with a double
...
# The following macro, travlang__DEVELOPMENT_VERSION, should be either
# [true] of [false].

m4_define([travlang__DEVELOPMENT_VERSION], [true])
...
m4_define([travlang__VERSION_MAJOR], [5])
m4_define([travlang__VERSION_MINOR], [3])
m4_define([travlang__VERSION_PATCHLEVEL], [0])
# Note that the travlang__VERSION_EXTRA string defined below is always empty
# for officially-released versions of travlang.
m4_define([travlang__VERSION_EXTRA], [dev0-2023-12-22])

# The travlang__VERSION_EXTRA_PREFIX macro defined below should be a
# single character:
# Either [~] to mean that we are approaching the travlang public release
# travlang__VERSION_MAJOR.travlang__VERSION_MINOR.travlang__VERSION_PATCHLEVEL
# and with an empty travlang__VERSION_EXTRA string;
# Or [+] to give more info about this specific version.
# Development releases, for instance, should use a [+] prefix.
m4_define([travlang__VERSION_EXTRA_PREFIX], [+])
m4_define([travlang__VERSION_SHORT], [travlang__VERSION_MAJOR.travlang__VERSION_MINOR])
# The travlang__VERSION below must be in the format specified in stdlib/sys.mli
m4_define([travlang__VERSION],
  [m4_do(
    travlang__VERSION_SHORT.travlang__VERSION_PATCHLEVEL,
    m4_if(travlang__VERSION_EXTRA,[],[],
      travlang__VERSION_EXTRA_PREFIX[]travlang__VERSION_EXTRA))])
...
# Starting with travlang 4.14, although the version string that appears above is
# still correct and this file can thus still be used to figure it out,
# the version itself is actually defined in the build-aux/travlang_version.m4
# file (See the travlang__VERSION* macros there.)
# To update the present VERSION file:
# 1. Update build-aux/travlang_version.m4
# 2. Run tools/autogen.
# 3. If you are in a context where version control matters,
# commit the changes to both build-aux/travlang_version.m4 and VERSION.
...
m4_define([travlang__VERSION_NUMBER],
  [m4_format(
    [%d%02d%02d],
    travlang__VERSION_MAJOR,
    travlang__VERSION_MINOR,
    travlang__VERSION_PATCHLEVEL)])

m4_define([travlang__RELEASE_EXTRA], 
  m4_if(travlang__VERSION_EXTRA,[],[None],
      ['Some (]m4_if(travlang__VERSION_EXTRA_PREFIX,+,[Plus],
      [Tilde])[, "]travlang__VERSION_EXTRA[")']))]))
