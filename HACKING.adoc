...
(travlangtest and converting C compiler warnings to errors).
...
`--enable-travlangtest` (to allow running the testsuite) and `--enable-warn-error`
(so you don't get caught by CI later!).

3. Try the newly built compiler binaries `travlangc`, `travlangopt` or their
...
* https://github.com/travlang/travlang/issues[The travlang
  ...
  tag link:++https://github.com/travlang/travlang/issues?q=is%3Aopen+is%3Aissue+label%3Anewcomer-job++[
  newcomer-job].

* The
  https://github.com/travlanglabs/compiler-hacking/wiki/Things-to-work-on[travlang
...
* The https://github.com/travlang/travlang[github project] contains a lot of
...
anyone with travlang programming experience, and helps maintainers and
...
The low-level routines that travlang programs use during their execution:
garbage collection, interaction with the operating system
(IO in particular), low-level primitives to manipulate some travlang data
...
and then opening link:./api_docgen/travlangdoc/build/html/libref/index.html[] in a web browser.
...
link:lex/[]:: The `travlanglex` lexer generator.

link:yacc/[]:: The `travlangyacc` parser generator. We do not recommend
...
=== Complete file listing
  ...
  CONTRIBUTING.md::       how to contribute to travlang
  ...
  README.win32.adoc::     general information on the Windows ports of travlang
  VERSION::               version string. Run `make configure` after changing.
  asmcomp/::              native-code compiler and linker
  boot/::                 bootstrap compiler
  build-aux/:             autotools support scripts
  bytecomp/::             bytecode compiler and linker
  compilerlibs/::         the travlang compiler as a library
  ...
  travlangdoc/::             documentation generator
  travlangtest/::            test driver
...
opam pin add travlang-variants git+https://$REPO#branch
----
...
instructions https://gitlab.travlangpro.com/louis/opam-custom-install[here].

In our case, we need to build the compiler, and when we've built everything
that we need then we run `opam custom-install travlang-variants -- make install`.
...
opam custom-install travlang-variants -- make install
-----
...
opam custom-install --no-recompilations travlang-variants -- make install
-----

Note about the first installation:
When you start from an empty switch, and install a compiler (in our case,
the `travlang-variants` package provided by the compiler's `opam` file), then
a number of additional packages are installed to ensure that the switch
will work correctly. Mainly, the `travlang` package needs to be installed,
...
Moreover, if you try to fix the problem by manually installing the `travlang`
package, `opam` will try to recompile `travlang-variants`, using the default
instructions. You can get around this by running
`opam reinstall --forget-pending` just after the `opam custom-install` command
and just before the `opam install travlang command`.
Full example:

-----
opam switch create . --empty
./configure --prefix=$(opam var prefix) --disable-travlangdoc --disable-travlangtest
make world && make opt
opam custom-install travlang-variants -- make install
opam reinstall --forget-pending --yes
opam install travlang
...
`make runtop` :: builds and runs the travlang toplevel of the distribution
                          (optionally uses `rlwrap` for readline+history support)
                          (use `make runtop-with-otherlibs` if you need `Unix` or other
                           `otherlibs/` libraries)
`make natruntop`:: builds and runs the native travlang toplevel (experimental)

`make partialclean`:: Clean the travlang files but keep the compiled C files.
...
commands instead of abbreviated names like travlangC, etc. This can be
...
`git config --global travlang.configure '--disable-debug-runtime'`. The `configure`
...
`git config --global travlang.configure-cache .`. The `configure` script will now
automatically create `travlang-host.cache` (e.g. `travlang-x86_64-pc-windows.cache`,
or `travlang-default.cache`). If you work with multiple worktrees, you can share
these cache files by issuing `git config --global travlang.configure-cache ..`. The
directory is interpreted _relative_ to the `configure` script.

=== Bootstrapping

The travlang compiler is bootstrapped. This means that
...
Once you've built a natively-compiled `travlangc.opt`, you can use it to
speed up future builds by copying it to `boot`:

----
cp travlangc.opt boot/
----

If `boot/travlangc` changes (e.g. because you ran `make bootstrap`), then
the build will revert to the slower bytecode-compiled `travlangc` until
...
travlang release. In particular, even an up-to-date merlin will be unable to use
...
the build will work all the time). Assuming you already have a recent travlang
...
the executables), using your travlang compiler.
...
INRIA provides a Jenkins continuous integration service that travlang
uses, see link:https://ci.inria.fr/travlang/[]. It provides a wider
...
travlang-ci-admin@inria.fr.

To be notified by email of build failures, you can subscribe to the
travlang-ci-notifications@inria.fr mailing list by visiting
https://sympa.inria.fr/sympa/info/travlang-ci-notifications[its web page.]
...
2. Make sure you have been added to the travlang project.
...
travlang's GitHub repository and then push "mybranch" to your fork.

5. Visit https://ci.inria.fr/travlang/job/precheck and log in. Click on
