...
(* A few extensions to travlang's standard library *)

module Unix = travlangtest_unix
...
module Filename = struct
  ...
  let mkexe filename = filename ^ travlangtest_config.exe
end
...
module Sys = struct
  ...
  let erase_file path =
    try Sys.remove path
    with Sys_error _ when Sys.win32 && travlangtest_config.libunix <> None ->
