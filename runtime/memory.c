...
/* Note [MM]: Enforcing the memory model.
   ...
   Multicore travlang implements the memory consistency model defined in
   ...
   and C++ SC atomics for atomic ones is not enough, since the travlang
   memory model is stronger. The prototypical example where C++
   exhibits a behaviour not allowed by travlang is below. Assume that the
   ...
   This outcome is not permitted by the travlang memory model, as can be
   ...
   optimising writes because reads are vastly more common). The travlang
   ...
   This encodes the travlang memory model in the primitives provided by
   the C++ model.
...
/* Note [MMMOC]: Mixing the Memory Models of travlang and C.

   Note [MM] above document how the code generated by the travlang
   ...
   pure travlang programs.

   On the other hand, hybrid travlang/C programs are written in two
   ...
   that is written using the travlang FFI, typically in user libraries,
   but also the C code of the travlang runtime itself.
   ...
   - But for the travlang heap and any other data that is accessed both
     from the C runtime and from the travlang mutator, we currently use
     ...
     (https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2020/p0124r7.html):
       ...
       1. Using C11 atomics does not provide any correctness
          guarantees in presence of races coming from travlang accesses, we
       ...
       4. But in practice many usage patterns of [volatile *]
          ...
          dangerous patterns are unlikely to be met in real-life travlang
          FFI code. We currently use a [volatile *] cast in Field
          for this reason.

   Note that these recommendations do not constitute a proper memory
   model for mixed travlang/C programs. To be used safely, they should
...
CAMLexport CAMLweakdef void caml_modify (volatile value *fp, value val)
{
   ...
   * travlang-multicore/travlang-tsan/pull/22#issuecomment-1377439074 on Github).
   */
